...
abstract class ModuleCompiler
{
    ...
    protected function compileGetParent(JsCompiler $compiler, \Twig_NodeInterface $node)
    {
        $compiler
            ->write("/**\n", " * @inheritDoc\n", " */\n")
            ->write(sprintf($this->functionNameTemplate, $this->functionName) . ".prototype.getParent_ = function(context) {\n")
            ->indent()
            ->write('return ')
        ;
    ...
    protected function compileDisplayHeader(JsCompiler $compiler, \Twig_NodeInterface $node)
    {
        $compiler
            ->write("/**\n", " * @inheritDoc\n", " */\n")
            ->write(sprintf($this->functionNameTemplate, $this->functionName).".prototype.render_ = function(sb, context, blocks) {\n")
            ->indent()
    ...
    protected function compileGetTemplateName(JsCompiler $compiler, \Twig_NodeInterface $node)
    {
        $compiler
            ->write("/**\n", " * @inheritDoc\n", " */\n")
            ->write(sprintf($this->functionNameTemplate, $this->functionName).".prototype.getTemplateName = function() {\n")
            ->indent()
    ...
    protected function compileIsTraitable(JsCompiler $compiler, \Twig_NodeInterface $node)
    {
        ...
        $compiler
            ->write(
                ...
                " * @return {boolean}\n",
                " */\n"
            )->write(sprintf($this->functionNameTemplate, $this->functionName).".prototype.isTraitable = function() {\n")
            ->indent()
